...
def create_appointment():
    ...
    if request.method == 'POST':
        ...
        query = "SELECT ap.appointment_time, ap.id, u.name, u.contact from appointments as ap join user_profiles as u on ap.user_id=u.id where ap.consultation_end is null and user_id in ({user_ids}) and DATE(ap.appointment_time) = %s order by appointment_time".format(
            user_ids=ids_string)
